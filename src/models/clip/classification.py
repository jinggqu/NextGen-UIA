...
from src.third_party.openai_clip.clip_adapter import CLIPAdapter
from monai.losses import FocalLoss
from matplotlib import pyplot as plt
from src.utils.tools import MetricAccumulator, model_summary, setup_logging, plot_roc_curve
from src.adapters import inject_mona_variant_to_clip, inject_lora_to_clip
...
def get_args():
    ...
    # Data related
    parser.add_argument("--exp", type=str, default="clip_cls")
    ...
    parser.add_argument("--mona_variant", type=str, default="freq_enhanced", help="MONA variant")
...
def prepare_model(args):
    ...
    if args.mona_weights:
        # Load checkpoint to determine adapter type
        checkpoint = torch.load(args.mona_weights, map_location="cpu", weights_only=True)
        adapter_type = checkpoint.get("adapter_type", None)
        ...
        if adapter_type == "lora":
            ...
            clip_model, lora_count = inject_lora_to_clip(
                clip_model, lora_r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, num_layers=args.mona_layers
            )
            ...
            logging.info(f"✓ Loaded {lora_params_loaded} pretrained LoRA parameters from {args.mona_weights}")
        else:
