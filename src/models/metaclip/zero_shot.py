...
def get_args():
    ...
    parser = argparse.ArgumentParser("Zero-shot Classification with MetaCLIP")

    # Data related
    parser.add_argument("--exp", type=str, default="metaclip_zero_shot")
...
def prepare_model(args):
    ...
    if args.mona_weights:
        ...
        # The checkpoint is the mona_state_dict itself (saved directly in finetune.py)
...
def test(args):
    logging.info("=" * 50)
    logging.info("MetaCLIP Zero-shot Classification")
    logging.info("=" * 50)
    ...
    plt.title(f"MetaCLIP Zero-shot\nAUC = {stats['auc']:.4f}", fontsize=14)
    plt.tight_layout()

    fig_name = "roc_curve_metaclip_zero_shot"
    ...
    result_str = "=" * 50 + "\n"
    result_str += "MetaCLIP Zero-shot Results\n"
    result_str += "=" * 50 + "\n"
