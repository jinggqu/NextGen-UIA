...
class USDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_path = os.path.join("../data/NextGen-UIA/all/masks", self.image_names[idx])

        image = Image.open(image_path).convert("L")
        label = Image.open(label_path).convert("1")  # Convert to binary mask
        ...
        if image.size != self.args.img_size:
            ...
            label = label.resize((self.args.img_size, self.args.img_size))
